...
| `demo.html` | 로컬/프리뷰 테스트용(실제 Cafe24 구조 재현). |
| `index.html` | GitHub Pages 루트 진입점(→ `demo.html`). |
| `assets/products/` | 데모용 실제 상품 사진 12장(타 도메인 프리뷰의 CORS 우회). |

## Cafe24 적용 (3단계)

1. **JS 업로드** — `[디자인 관리 > 파일 업로더]` 로 `wardrobe-site.js` 를 스킨 루트에 업로드.
   업로드 경로에 맞춰 `snippet.html` 의 `src="/wardrobe-site.js"` 만 수정.
2. **HTML 한 줄 추가** — 메인 페이지 템플릿 `</body>` 직전에 `snippet.html` 내용 붙여넣기.
...
## 공개 미리보기 (GitHub Pages)

`.github/workflows/pages.yml` 가 `main` push 시 저장소를 GitHub Pages 로 배포합니다.

1. 저장소 **Settings → Pages → Build and deployment → Source** 를 **“GitHub Actions”** 로 설정.
2. `main` 에 머지(또는 Actions 탭에서 워크플로 수동 실행)하면 배포됩니다.
3. 미리보기 URL: **https://kugu4239-create.github.io/merryon-wardrobe/**

## 이미지 경로와 CORS

- **운영(merryon.co.kr)**: `//merryon.co.kr/web/product/medium/...` (same-origin) — CORS 무관, 원본 경로 그대로 사용.
- **타 도메인 프리뷰(GitHub Pages·localhost)**: merryon 과 출처가 달라 CORS 로 텍스처 로드가
  실패할 수 있으므로, 데모는 저장소에 동봉한 `assets/products/` 의 사진을 로컬에서 불러옵니다.
  `demo.html` 이 `window.MERRYON_WARDROBE_CONFIG.resolveImage` 로 경로만 치환하며,
  **운영 코드(`wardrobe-site.js`)의 기본 동작(same-origin 원본 경로)은 변경되지 않습니다.**
- 어떤 이유로든 텍스처 로드가 실패하면 `tint`(브랜드 팔레트 색) 폴백으로 자연스럽게 대체됩니다.
...
- 호스트 HTML 에 importmap 을 넣을 필요가 없도록, `wardrobe-site.js` 가 실행 시
  ...
  덕분에 Cafe24 템플릿에는 본 스크립트 **한 줄만** 추가하면 됩니다.
  (three r160 부터 UMD `build/three.min.js` 가 CDN 에서 삭제되어 404 가 나므로 사용하지 않습니다.)
