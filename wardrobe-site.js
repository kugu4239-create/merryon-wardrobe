...
(function () {
  ...
  WardrobeScene.BUILD = { time: '06-18 10:00 UTC', note: 'CTA 도트 더 진하게(132,170,144)' };
  ...
  P._buildWritingStool = function () {
    ...
    // 시즌 기프트 안내 문구 — 2026-06-30 23:59(KST)까지만, 이후 원래 문구로 자동 복귀
    var _giftUntil = Date.parse('2026-06-30T23:59:59+09:00');
    var _writingMsg = (Date.now() <= _giftUntil)
      ? 'Emliy 고객님의 시즌 기프트 신청이 가능합니다'
      : '라운지의 서비스가 준비 중입니다.';
    this._registerHotspot('writing', g, new T.Vector3(0, 0.5, 0.0), 0.82, 45, _writingMsg);    // 탭 → 종이/펜 근접 (중상단 문구: 개별)
    ...
    if (Date.now() <= _giftUntil) {
      ...
      dot.position.y = -0.10; dot.castShadow = true; bang.add(dot);
      bang.rotation.y = -0.25;   // 스툴 정면쪽으로 살짝
    }
  };
  ...
  P._startCTAAnim = function () {
    ...
    var R = 112, step = 10, maxR = 3.7, k = 0.10, speed = 4.2, col = '132,170,144';   // 세이지민트(더 진하게)
    ...
    var draw = function (now) {
      ...
      for (var gy = -R; gy <= R; gy += step) {
        for (var gx = -R; gx <= R; gx += step) {
          ...
          var fore = nz * (0.55 + 0.45 * nz);                         // 곡률 포어쇼트닝(가장자리로 급격히 작게)
          var wave = 0.74 + 0.26 * Math.sin(r2 * R * k - t * speed);  // 안→밖 파동(곡면 따라)
          var rr = maxR * fore * wave;
          if (rr < 0.3) continue;
